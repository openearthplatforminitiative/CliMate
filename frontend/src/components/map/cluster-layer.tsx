  ...
  const handleClusterClick = useCallback((e: MapMouseEvent) => {
    ...
    if (clusterSource && clusterSource.type === 'geojson') {
      ...
      source.getClusterLeaves(clusterId, pointCount, 0).then((leaves) => {
        setClickedCluster({
          point: new LngLat(clusterGeometry.coordinates[0], clusterGeometry.coordinates[1]),
  ...
  return (
    <>
      ...
      {clickedCluster && clickedCluster.issues.length > 1 && (
        ...
        >
          <DropdownMenu>
            <DropdownMenuTrigger asChild><Button variant="outline">Issues<ChevronDown /></Button></DropdownMenuTrigger>
